...
def confirmDelete():
  ...
  window.configure(bg = 'white')
  window.resizable(False, False)
  label = tk.Label(window, font = ("./font/NanumGothicBold.ttf", 25), bg = 'white', text = "Are you sure to Delete?")
  label.place(y=20, relx = 0.5, anchor='n')
  yesB = tk.Button(window, width=15, height= 2, relief="raised", overrelief="solid", borderwidth=4, font = ("./font/NanumGothic.ttf", 15), text= "Yes", command = lambda: close_window2(window, True))
  yesB.place(y = 75, relx=0.25, anchor='n')
  noB = tk.Button(window, width=15, height= 2, relief="raised", overrelief="solid", borderwidth=4, font = ("./font/NanumGothic.ttf", 15), text= "No", command = lambda: close_window2(window, False))
  noB.place(y = 75, relx=0.75, anchor='n')
  window.mainloop()
          ...
          if event.type == KEYDOWN:
            if event.key == ord('q'):
              ...
              if event.key in K_NUM or event.key == K_BACKSPACE:
                ...
                lstBoxBlinds[selectedIdx][1][lstBoxBlinds[selectedIdx][2]].changeContent(font = fontBox, content = str_input)
                tabbedOnly = False
                ...
                if selectedIdx == len(lstBoxBlinds)-1 and lstBoxBlinds[selectedIdx][2] == 2:
                  ...
                  if not tabbedOnly:
                    ...
                    str_input = f"{temp_input:,}"
                    lstBoxBlinds[selectedIdx][1][2].changeContent(font = fontBox, content = str_input)
                    objControl.changeLstBlinds(selectedIdx, lstBoxBlinds[selectedIdx][2] - 1, temp_input)
                  selectedIdx = -1
